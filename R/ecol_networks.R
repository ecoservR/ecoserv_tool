...
#' @threshold Functional size threshold (in hectares) under which a mapped network is considered too small to serve its purpose. Default is 1 ha.
...
create_network <- function(x = parent.frame()$mm,
   ...
   costdistance <- accCost(tr, source_pts)
   ...
   costdistance <- mask(costdistance, studyArea)
   ...
   core <- sf::st_as_sf(
      stars::st_as_stars(
         reclassify(costdistance,
                    c(0,1,1,
                      1, Inf, NA))
      ),
      as_points = FALSE,
      merge = TRUE) %>%
      dplyr::mutate(area_ha = as.numeric(sf::st_area(.))/10000) %>% # calculate area of networks
      dplyr::filter(area_ha > threshold) # remove those smaller than functional threshold

   meso <- sf::st_as_sf(
      stars::st_as_stars(
         reclassify(costdistance,  # combine core and meso
                    c(0,2,1,
                      2, Inf, NA))
      ),
      as_points = FALSE,
   ...
   macro <- sf::st_as_sf(
      stars::st_as_stars(
         reclassify(costdistance, # combine core, meso and macro
                    c(0,3,1,
                      3, Inf, NA))
      ),
      as_points = FALSE,
   ...
   raster::writeRaster(
      raster::mask(costdistance, sf::st_union(network_poly)),
